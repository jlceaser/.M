...
fn gen_expr(idx: i32) -> i32 {
    ...
    if kind == NK_CALL() {
        ...
        if str_eq(callee_name, "print") {
            ...
            emit_byte(OP_PRINT());
            emit_byte(OP_NIL()); // print returns void
            return 0;
        }
...
// ══════════════════════════════════════════════════════
// ── VIRTUAL MACHINE (stack-based bytecode executor) ──
// ══════════════════════════════════════════════════════
...
// Call frame stack (parallel arrays)
var frame_func: i32 = 0;    // function index
var frame_ip: i32 = 0;      // instruction pointer
var frame_base: i32 = 0;    // stack base for this frame
var frame_count: i32 = 0;

// Global variable storage
var vm_globals: i32 = 0;
...
fn init_vm() -> i32 {
    vm_stack = array_new(0);
    vm_top = 0;
    frame_func = array_new(0);
    frame_ip = array_new(0);
    frame_base = array_new(0);
    frame_count = 0;
    vm_globals = array_new(0);
    vm_global_count = 0;
    vm_output = "";
    // Pre-allocate stack
    var i: i32 = 0;
    while i < 1024 {
        array_push(vm_stack, 0);
        i = i + 1;
    }
    return 0;
}

fn vm_push(v: i32) -> i32 {
    if vm_top < array_len(vm_stack) {
        array_set(vm_stack, vm_top, v);
    } else {
        array_push(vm_stack, v);
    }
    vm_top = vm_top + 1;
    return 0;
}
...
fn vm_pop() -> i32 {
    vm_top = vm_top - 1;
    return array_get(vm_stack, vm_top);
}

fn vm_peek(dist: i32) -> i32 {
    return array_get(vm_stack, vm_top - 1 - dist);
}

fn vm_set(idx: i32, val: i32) -> i32 {
    array_set(vm_stack, idx, val);
    return 0;
}

// Read bytecode from current frame
fn read_byte_vm() -> i32 {
...
fn vm_run_func(fi: i32) -> i32 {
    ...
    while true {
        ...
        if op == OP_CONST_INT() {
            ...
            vm_push(array_get(pool, ci));
        } else if op == OP_CONST_STRING() {
            // For now, strings are stored as constant index (negated to distinguish)
            let ci: i32 = read_u16_vm();
            // Store string index as negative to distinguish from ints
            // We use -(ci+1) so 0 stays distinct
            vm_push(0 - ci - 1);
        } else if op == OP_TRUE() {
            vm_push(1);
        } else if op == OP_FALSE() {
            ...
            vm_push(array_get(vm_stack, base + slot));
        } else if op == OP_LOCAL_SET() {
            let slot: i32 = read_u16_vm();
            vm_set(base + slot, vm_peek(0));
        } else if op == OP_GLOBAL_GET() {
            let gi: i32 = read_u16_vm();
            if gi < vm_global_count {
                vm_push(array_get(vm_globals, gi));
            } else {
                vm_push(0);
            }
            ...
            while gi >= vm_global_count {
                if vm_global_count < array_len(vm_globals) {
                    array_set(vm_globals, vm_global_count, 0);
                } else {
                    array_push(vm_globals, 0);
                }
                vm_global_count = vm_global_count + 1;
            }
            array_set(vm_globals, gi, vm_peek(0));
        } else if op == OP_ADD() {
            let b: i32 = vm_pop();
            let a: i32 = vm_pop();
            vm_push(a + b);
            ...
            let a: i32 = vm_pop();
            if a == 0 { vm_push(1); } else { vm_push(0); }
        } else if op == OP_JUMP() {
            ...
            let callee: i32 = array_get(vm_stack, vm_top - 1 - argc);
            let target_params: i32 = array_get(mod_params, callee);

            // Remove callee from stack: shift args down
            var k: i32 = 0;
            while k < argc {
                vm_set(vm_top - 1 - argc + k, array_get(vm_stack, vm_top - argc + k));
                k = k + 1;
            }
            vm_top = vm_top - 1;
            ...
            // Reserve space for extra locals
            ...
            let v: i32 = vm_pop();
            if v < 0 {
                // Negative values are string constant indices: -(ci+1)
                let ci: i32 = 0 - v - 1;
                let cur_f: i32 = array_get(frame_func, frame_count - 1);
                let pool: i32 = array_get(mod_strs, cur_f);
                if ci < array_len(pool) {
                    vm_output = str_concat(vm_output, array_get(pool, ci));
                }
            } else {
                vm_output = str_concat(vm_output, int_to_str(v));
            }
        } else {
            // Unknown opcode — halt
            return 0 - 1;
